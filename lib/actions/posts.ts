  ...
  waitUntil(
    (async () => {
      const [comment] = await db
        ...
        .limit(1)
      if (comment) {
        await indexComment(comment, data.owner, data.repo, newPost.number, newPost.categoryId, true)
      }
    })()
  )
  ...
  waitUntil(
    (async () => {
      const [comment] = await db
        ...
        .limit(1)
      if (comment) {
        await indexComment(comment, post.owner, post.repo, post.number, post.categoryId, false)
      }
